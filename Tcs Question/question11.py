...
concentration
 of PMs include vehicle emission by applying Odd Even concept for all types of vehicles. The vehicles with the odd last digit in
 the registration number will be allowed on roads on odd dates and those with even last digit will on even dates.
...
n=int(input("enter the no of vechiles:"))
digit=set()
if n>0:
    for i in range(1,n+1):
        vno=int(input("enter the last digit of vechile no {} :".format(i)))
        if len(str(vno))==1:
            tot=digit.add(vno)
        else:
            print("enter only last digit number")
    print(digit)
    date=int(input("enter the date of month:"))
    if date > 0 :
        fine = int(input("enter the Rs to gave the  fine: "))
        if fine>0:
            totFine=0
            if date%2==0:
                count=0
                ...
                print("Fine collected Rs:{}".format(0))
        else:
            print("you can't write in negative date only between 1 nad 30")
    else:
        print("you can't write in negative vechile number")
