...
an array[] of N number of integer values. The task here is to sort the items based on their levels of risk in the array. The risk values
 range from 0 to 2.
Example :
Input :
7  -> Value of N
...
lst=[]
print("enter the no of category:")
for i in range(0,n):
    item=int(input())
    if 0<=item<=2:
        lst.append(item)
